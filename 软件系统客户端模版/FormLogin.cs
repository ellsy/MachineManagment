...
namespace 软件系统客户端模版
{
    ...
    public partial class FormLogin : Form
    {
        ...
        {
            MachineBase bill = new BillPressureVessel();
            MessageBox.Show(JObject.FromObject(bill).ToString());
            return;
        ...
        private void ThreadCheckAccount()
        {
            ...
            //下载设备信息
            using (SqlConnection conn = new SqlConnection(CommonLibrary.CommonLibrary.MachineSqlConn))
            {
                conn.Open();
                起重设备 = BasicFramework.SoftSqlOperate.ExecuteSelectEnumerable<BillCrane>(conn, BillCrane.SqlSelectedHead).ToList();
                电梯设备 = BasicFramework.SoftSqlOperate.ExecuteSelectEnumerable<BillElevator>(conn, BillElevator.SqlSelectedHead).ToList();
                一般设备台账 = BasicFramework.SoftSqlOperate.ExecuteSelectEnumerable<BillNormal>(conn, BillNormal.SqlSelectedHead).ToList();
                压力容器台账 = BasicFramework.SoftSqlOperate.ExecuteSelectEnumerable<BillPressureVessel>(conn, BillPressureVessel.SqlSelectedHead).ToList();
                锅炉台账 = BasicFramework.SoftSqlOperate.ExecuteSelectEnumerable<BillBoiler>(conn, BillBoiler.SqlSelectedHead).ToList();
                管道台账 = BasicFramework.SoftSqlOperate.ExecuteSelectEnumerable<BillPipe>(conn, BillPipe.SqlSelectedHead).ToList();
            }
        ...
        #region 需要下载的设备数据块
        public List<BillCrane> 起重设备 { get; set; } = new List<BillCrane>();
        public List<BillElevator> 电梯设备 { get; set; } = new List<BillElevator>();
        public List<BillNormal> 一般设备台账 { get; set; } = new List<BillNormal>();



        public List<BillPressureVessel> 压力容器台账 { get; set; } = new List<BillPressureVessel>();
        public List<BillBoiler> 锅炉台账 { get; set; } = new List<BillBoiler>();
        public List<BillPipe> 管道台账 { get; set; } = new List<BillPipe>();
        #endregion
    }
}
